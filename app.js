...
async function loadBudgets() {
    ...
    const { data: budgets, error } = await supabase.from('budgets').select('*').order('id', { ascending: true });
...
async function loadRunSheets() {
    ...
    const { data: runSheets, error } = await supabase.from('run_sheets').select('*').order('id', { ascending: true });
    if (error) { console.error('Error loading run-sheets:', error); return; }
    data.runSheets = runSheets || [];
}

async function loadTrainingProgress() {
    const { data: progress, error } = await supabase.from('training_progress').select('*').eq('user_email', currentUser.email);
    if (error) { console.error('Error loading training progress:', error); return; }
...
function renderEvents() {
    ...
    tbody.innerHTML = data.events.map(e => {
        ...
        return '<tr><td><strong>' + e.name + '</strong></td><td>' + e.client + '</td><td>' + e.date + '</td><td>' + e.venue + '</td><td><span class="status-badge status-' + e.status.toLowerCase() + '">' + e.status + '</span></td><td>' + actions + '</td></tr>';
    }).join('');
}
...
function renderVendors() {
    ...
    tbody.innerHTML = data.vendors.map(v => {
        const actions = isFounder ?
            ...
            '<span style="color:#94a3b8;font-size:12px;">View only</span>';
        return '<tr><td><strong>' + v.name + '</strong></td><td>' + v.category + '</td><td>' + v.contact + '</td><td>' + v.phone + '</td><td>' + v.price + '</td><td>' + actions + '</td></tr>';
    }).join('');
}
...
function renderTasks() {
    ...
    data.tasks.forEach(t => {
        ...
        let actions = '';
        if (canMove) {
            if (t.status === 'todo') actions += '<button class="btn-secondary" onclick="moveTask(' + t.id + ',\'progress\')">➡️ Start</button>';
            if (t.status === 'progress') actions += '<button class="btn-secondary" onclick="moveTask(' + t.id + ',\'done\')">✅ Done</button>';
        }
        const card = '<div class="task-card"><div class="task-title">' + t.title + '</div><div class="task-meta">' + (t.assigned_to || 'Unassigned') + (t.event_id ? ' • Event #' + t.event_id : '') + '</div><div class="task-actions">' + actions + '</div></div>';
...
function renderBudgets() {
    ...
    tbody.innerHTML = data.budgets.map(b => {
        ...
        return '<tr><td>' + eventName + '</td><td>' + b.category + '</td><td>₹' + b.estimated.toLocaleString() + '</td><td>₹' + b.actual.toLocaleString() + '</td><td style="color:' + diffColor + ';font-weight:' + (diff !== 0 ? '600' : '400') + ';">' + (diff > 0 ? '+' : '') + diff.toLocaleString() + '</td><td><span class="status-badge">' + b.status + '</span></td></tr>';
    }).join('');
}
...
function renderRunSheets() {
    ...
    container.innerHTML = data.runSheets.map(rs => {
        ...
        return '<div style="background:#fff;padding:20px;border-radius:12px;border:1px solid #e9edf4;margin-bottom:16px;"><h3 style="margin-bottom:4px;">' + eventName + '</h3><div style="font-size:13px;color:#64748b;white-space:pre-wrap;background:#f8fafc;padding:12px;border-radius:6px;margin-top:8px;">' + rs.timeline + '</div><div style="margin-top:8px;"><button class="btn-edit" onclick="editRunSheet(' + rs.id + ')">✏️ Edit</button><button class="btn-danger" onclick="deleteRunSheet(' + rs.id + ')">🗑️ Delete</button></div></div>';
    }).join('');
}
...
function renderTraining() {
    ...
    grid.innerHTML = data.training.modules.map(m => {
        ...
        return '<div class="training-card" onclick="toggleModule(' + m.id + ')"><h4>' + m.id + '. ' + m.title + '</h4><p>' + summary + '</p><div class="module-status ' + statusClass + '">' + statusText + '</div></div>';
    }).join('');
}
...
function renderTrainingProgress() {
    ...
    container.innerHTML = '<div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;width:100%;"><span style="font-weight:600;">Progress: ' + completed + '/' + total + ' modules (' + percentage + '%)</span><div style="flex:1;min-width:200px;height:8px;background:#e9edf4;border-radius:4px;overflow:hidden;"><div style="width:' + percentage + '%;height:100%;background:#3b82f6;border-radius:4px;"></div></div><span style="font-size:14px;color:#64748b;">' + percentage + '%</span></div>';
}
...
function showAddEventForm() {
    if (currentUserRole !== 'founder') { alert('Only founders can add events.'); return; }
    openModal('<h2>Add New Event</h2><label>Event Name</label><input id="f-event-name" placeholder="Sharma Wedding" /><label>Client Name</label><input id="f-event-client" placeholder="Mr. Sharma" /><label>Date</label><input id="f-event-date" type="date" /><label>Venue</label><input id="f-event-venue" placeholder="Grand Gardens" /><label>Status</label><select id="f-event-status"><option value="Upcoming">Upcoming</option><option value="Ongoing">Ongoing</option><option value="Completed">Completed</option></select><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="addEvent()">Save</button></div>');
}
...
async function addEvent() {
    ...
    const { data: newEvent, error } = await supabase.from('events').insert([{ name, client, date, venue, status, created_by: currentUser.email }]).select().single();
    if (error) { console.error('Error adding event:', error); alert('Failed to add event.'); return; }
...
async function deleteEvent(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can delete events.'); return; }
    if (!confirm('Delete this event?')) return;
    const { error } = await supabase.from('events').delete().eq('id', id);
    if (error) { console.error('Error deleting event:', error); alert('Failed to delete event.'); return; }
    data.events = data.events.filter(e => e.id !== id);
    renderAll();
}

function editEvent(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can edit events.'); return; }
    const e = data.events.find(ev => ev.id === id);
    if (!e) return;
    openModal('<h2>Edit Event</h2><label>Event Name</label><input id="f-event-name" value="' + e.name + '" /><label>Client Name</label><input id="f-event-client" value="' + e.client + '" /><label>Date</label><input id="f-event-date" type="date" value="' + e.date + '" /><label>Venue</label><input id="f-event-venue" value="' + e.venue + '" /><label>Status</label><select id="f-event-status"><option value="Upcoming" ' + (e.status === 'Upcoming' ? 'selected' : '') + '>Upcoming</option><option value="Ongoing" ' + (e.status === 'Ongoing' ? 'selected' : '') + '>Ongoing</option><option value="Completed" ' + (e.status === 'Completed' ? 'selected' : '') + '>Completed</option></select><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="updateEvent(' + id + ')">Update</button></div>');
}
...
async function updateEvent(id) {
    ...
    const { data: updatedEvent, error } = await supabase.from('events').update(updatedData).eq('id', id).select().single();
    if (error) { console.error('Error updating event:', error); alert('Failed to update event.'); return; }
...
function showAddVendorForm() {
    if (currentUserRole !== 'founder') { alert('Only founders can add vendors.'); return; }
    openModal('<h2>Add New Vendor</h2><label>Vendor Name</label><input id="f-vendor-name" placeholder="Grand Gardens" /><label>Category</label><select id="f-vendor-category"><option value="Venue">Venue</option><option value="Caterer">Caterer</option><option value="Photographer">Photographer</option><option value="Decorator">Decorator</option><option value="DJ">DJ</option><option value="Makeup Artist">Makeup Artist</option></select><label>Contact Person</label><input id="f-vendor-contact" placeholder="Mr. Rajesh" /><label>Phone</label><input id="f-vendor-phone" placeholder="98765XXXXX" /><label>Price</label><input id="f-vendor-price" placeholder="₹50,000" /><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="addVendor()">Save</button></div>');
}
...
async function addVendor() {
    ...
    const { data: newVendor, error } = await supabase.from('vendors').insert([{ name, category, contact, phone, price, created_by: currentUser.email }]).select().single();
    if (error) { console.error('Error adding vendor:', error); alert('Failed to add vendor.'); return; }
...
async function deleteVendor(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can delete vendors.'); return; }
    if (!confirm('Delete this vendor?')) return;
    const { error } = await supabase.from('vendors').delete().eq('id', id);
    if (error) { console.error('Error deleting vendor:', error); alert('Failed to delete vendor.'); return; }
    data.vendors = data.vendors.filter(v => v.id !== id);
    renderAll();
}

function editVendor(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can edit vendors.'); return; }
    const v = data.vendors.find(vendor => vendor.id === id);
    if (!v) return;
    openModal('<h2>Edit Vendor</h2><label>Vendor Name</label><input id="f-vendor-name" value="' + v.name + '" /><label>Category</label><select id="f-vendor-category"><option value="Venue" ' + (v.category === 'Venue' ? 'selected' : '') + '>Venue</option><option value="Caterer" ' + (v.category === 'Caterer' ? 'selected' : '') + '>Caterer</option><option value="Photographer" ' + (v.category === 'Photographer' ? 'selected' : '') + '>Photographer</option><option value="Decorator" ' + (v.category === 'Decorator' ? 'selected' : '') + '>Decorator</option><option value="DJ" ' + (v.category === 'DJ' ? 'selected' : '') + '>DJ</option><option value="Makeup Artist" ' + (v.category === 'Makeup Artist' ? 'selected' : '') + '>Makeup Artist</option></select><label>Contact Person</label><input id="f-vendor-contact" value="' + v.contact + '" /><label>Phone</label><input id="f-vendor-phone" value="' + v.phone + '" /><label>Price</label><input id="f-vendor-price" value="' + v.price + '" /><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="updateVendor(' + id + ')">Update</button></div>');
}
...
async function updateVendor(id) {
    ...
    const { data: updatedVendor, error } = await supabase.from('vendors').update(updatedData).eq('id', id).select().single();
    if (error) { console.error('Error updating vendor:', error); alert('Failed to update vendor.'); return; }
...
function showAddTaskForm() {
    if (currentUserRole !== 'founder') { alert('Only founders can create tasks.'); return; }
    const eventOptions = data.events.map(e => '<option value="' + e.id + '">' + e.name + '</option>').join('');
    openModal('<h2>Add New Task</h2><label>Task Title</label><input id="f-task-title" placeholder="Call Caterer" /><label>Description</label><textarea id="f-task-desc" placeholder="Confirm December 15 availability"></textarea><label>Assigned To</label><select id="f-task-assign"><option value="Intern 1">Intern 1</option><option value="Intern 2">Intern 2</option><option value="Intern 3">Intern 3</option></select><label>Related Event</label><select id="f-task-event"><option value="">None</option>' + eventOptions + '</select><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="addTask()">Save</button></div>');
}
...
async function addTask() {
    ...
    const { data: newTask, error } = await supabase.from('tasks').insert([{ title, description, assigned_to, event_id, status: 'todo', created_by: currentUser.email }]).select().single();
    if (error) { console.error('Error adding task:', error); alert('Failed to add task.'); return; }
...
async function moveTask(id, newStatus) {
    ...
    if (!task) return;
    const { data: updatedTask, error } = await supabase.from('tasks').update({ status: newStatus }).eq('id', id).select().single();
    if (error) { console.error('Error moving task:', error); alert('Failed to update task.'); return; }
...
function showAddBudgetForm() {
    if (currentUserRole !== 'founder') { alert('Only founders can manage budgets.'); return; }
    const eventOptions = data.events.map(e => '<option value="' + e.id + '">' + e.name + '</option>').join('');
    openModal('<h2>Add Budget Item</h2><label>Event</label><select id="f-budget-event">' + eventOptions + '</select><label>Category</label><select id="f-budget-cat"><option value="Venue">Venue</option><option value="Catering">Catering</option><option value="Photography">Photography</option><option value="Decor">Decor</option><option value="DJ">DJ</option><option value="Miscellaneous">Miscellaneous</option></select><label>Estimated Cost (₹)</label><input id="f-budget-est" type="number" placeholder="50000" /><label>Actual Cost (₹)</label><input id="f-budget-act" type="number" placeholder="48000" /><label>Status</label><select id="f-budget-status"><option value="Not Paid">Not Paid</option><option value="Partially Paid">Partially Paid</option><option value="Paid">Paid</option></select><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="addBudget()">Save</button></div>');
}
...
async function addBudget() {
    ...
    const { data: newBudget, error } = await supabase.from('budgets').insert([{ event_id, category, estimated, actual, status, created_by: currentUser.email }]).select().single();
    if (error) { console.error('Error adding budget:', error); alert('Failed to add budget item.'); return; }
...
function showAddRunSheetForm() {
    if (currentUserRole !== 'founder') { alert('Only founders can manage run-sheets.'); return; }
    const eventOptions = data.events.map(e => '<option value="' + e.id + '">' + e.name + '</option>').join('');
    openModal('<h2>Add Run-Sheet</h2><label>Event</label><select id="f-rs-event">' + eventOptions + '</select><label>Timeline (one line per item)</label><textarea id="f-rs-timeline" rows="6" placeholder="8:00 AM – Venue Walkthrough&#10;10:00 AM – Decor Setup&#10;6:00 PM – Ceremony"></textarea><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="addRunSheet()">Save</button></div>');
}
...
async function addRunSheet() {
    ...
    const { data: newRunSheet, error } = await supabase.from('run_sheets').insert([{ event_id, timeline, created_by: currentUser.email }]).select().single();
    if (error) { console.error('Error adding run-sheet:', error); alert('Failed to add run-sheet.'); return; }
...
async function deleteRunSheet(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can delete run-sheets.'); return; }
    if (!confirm('Delete this run-sheet?')) return;
    const { error } = await supabase.from('run_sheets').delete().eq('id', id);
    if (error) { console.error('Error deleting run-sheet:', error); alert('Failed to delete run-sheet.'); return; }
    data.runSheets = data.runSheets.filter(rs => rs.id !== id);
    renderAll();
}

function editRunSheet(id) {
    if (currentUserRole !== 'founder') { alert('Only founders can edit run-sheets.'); return; }
    ...
    openModal('<h2>Edit Run-Sheet</h2><label>Event</label><select id="f-rs-event">' + eventOptions + '</select><label>Timeline (one line per item)</label><textarea id="f-rs-timeline" rows="6">' + rs.timeline + '</textarea><div class="form-actions"><button class="btn-secondary" onclick="closeModal()">Cancel</button><button class="btn-primary" onclick="updateRunSheet(' + id + ')">Update</button></div>');
}
...
async function updateRunSheet(id) {
    ...
    const { data: updatedRunSheet, error } = await supabase.from('run_sheets').update(updatedData).eq('id', id).select().single();
    if (error) { console.error('Error updating run-sheet:', error); alert('Failed to update run-sheet.'); return; }
